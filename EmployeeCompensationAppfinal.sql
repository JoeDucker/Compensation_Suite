...
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'EmployeeDemoApp')
BEGIN
    ALTER DATABASE EmployeeDemoApp SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE EmployeeDemoApp;
END
GO
...
-- Optional: drop the old proc if it exists
IF OBJECT_ID('dbo.usp_UpdateCompensationWithAudit', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_UpdateCompensationWithAudit;
GO

CREATE PROCEDURE dbo.usp_UpdateCompensationWithAudit
    @Employee_ID       VARCHAR(10),
    @NewCompensation   DECIMAL(18,2),
    @NewActive         CHAR(1),                   -- 'Y' or 'N'
    @Reason            NVARCHAR(400) = NULL,
    @ChangedBy         NVARCHAR(128) = NULL,      -- pass Environment.UserName
    @MachineName       NVARCHAR(128) = NULL,      -- pass Environment.MachineName
    @AppName           NVARCHAR(128) = NULL,      -- pass Application.ProductName
    @ReferenceDate     DATE          = NULL       -- defaults to today if NULL
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        SET @ReferenceDate = CONVERT(date, GETDATE());
    END
    ...
    BEGIN TRY
        ...
        (
            SELECT TOP (1) *
            FROM dbo.Employee_Assignment
            WHERE Employee_ID = @Employee_ID
            ORDER BY Reference_Date DESC, Assignment_ID DESC
        )
        SELECT *
        INTO #cur
        FROM Latest;
        ...
        DECLARE
            @OldAssignmentID INT          = (SELECT Assignment_ID FROM #cur),
            @OldComp         DECIMAL(18,2)= (SELECT Compensation  FROM #cur),
            @OldActive       CHAR(1)      = (SELECT Active        FROM #cur),
            @RoleID          INT          = (SELECT Role_ID       FROM #cur),
            @LocationID      INT          = (SELECT Location_ID   FROM #cur);

        /* Close the old row if it exists */
        IF @OldAssignmentID IS NOT NULL
        BEGIN
            UPDATE dbo.Employee_Assignment
            SET Active = 'N'
            WHERE Assignment_ID = @OldAssignmentID;
        END

        /* Insert the NEW assignment row (preserves Role/Location) */
        INSERT INTO dbo.Employee_Assignment
            (Employee_ID, Role_ID, Location_ID, Compensation, Active, Reference_Date)
        VALUES
            (@Employee_ID, @RoleID, @LocationID, @NewCompensation, @NewActive, @ReferenceDate);

    /* ---------- 1) Ensure the audit table exists & has the columns your app uses ---------- */
    ...
    BEGIN
        ...
        (
            ...
            Audit_Ts          DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
            ...
            App_Name          NVARCHAR(128)     NULL,  -- your app sends this
            Reason            NVARCHAR(400)     NULL
        );
    END
    ELSE
    BEGIN
        IF COL_LENGTH('dbo.Compensation_Audit','Audit_Ts') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Audit_Ts DATETIME2(0) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Employee_ID') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Employee_ID VARCHAR(10) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Old_Compensation') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Old_Compensation DECIMAL(18,2) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','New_Compensation') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD New_Compensation DECIMAL(18,2) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Old_Active') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Old_Active CHAR(1) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','New_Active') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD New_Active CHAR(1) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Role_ID') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Role_ID INT NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Location_ID') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Location_ID INT NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Changed_By') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Changed_By NVARCHAR(128) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Machine_Name') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Machine_Name NVARCHAR(128) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','App_Name') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD App_Name NVARCHAR(128) NULL;
        IF COL_LENGTH('dbo.Compensation_Audit','Reason') IS NULL
            ALTER TABLE dbo.Compensation_Audit ADD Reason NVARCHAR(400) NULL;

        -- default for Audit_Ts if it was just added
        IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Compensation_Audit') AND name = 'Audit_Ts' AND is_nullable = 1)
        BEGIN
            UPDATE dbo.Compensation_Audit SET Audit_Ts = ISNULL(Audit_Ts, SYSUTCDATETIME());
            DECLARE @sql nvarchar(max) =
                N'ALTER TABLE dbo.Compensation_Audit ADD CONSTRAINT DF_CompAudit_AuditTs DEFAULT SYSUTCDATETIME() FOR Audit_Ts;';
            IF NOT EXISTS (
                SELECT 1 FROM sys.default_constraints
                WHERE parent_object_id = OBJECT_ID('dbo.Compensation_Audit') AND name = 'DF_CompAudit_AuditTs'
            )
                EXEC (@sql);
        END
    END

    /* ---------- 2) Drop & recreate the stored procedure without TRY/CATCH ---------- */

        /*------------------------------------------------------------
      0) Safety: create the audit table if it's missing
    ------------------------------------------------------------*/
    IF OBJECT_ID('dbo.Compensation_Audit','U') IS NULL
    BEGIN
        CREATE TABLE dbo.Compensation_Audit
    ...
    /*------------------------------------------------------------
      1) Stored proc used by the app's "Edit" and "Test Audit" paths
         - Updates the most recent assignment row for the employee
         - Writes an audit row to dbo.Compensation_Audit
    ------------------------------------------------------------*/
    CREATE OR ALTER PROCEDURE dbo.usp_UpdateCompensationWithAudit
        @Employee_ID      VARCHAR(10),
        @NewCompensation  DECIMAL(18,2),
        @NewActive        CHAR(1),
        @Reason           NVARCHAR(400)  = NULL,
        ...
        @ReferenceDate    DATE           = NULL  -- defaults to today inside
    AS
    BEGIN
        SET NOCOUNT ON;

        DECLARE 
            @OldComp      DECIMAL(18,2),
            @OldActive    CHAR(1),
            @RoleID       INT,
            @LocationID   INT,
            @refDate      DATE;

        SET @refDate = ISNULL(@ReferenceDate, CAST(GETDATE() AS DATE));

        BEGIN TRY
            BEGIN TRAN;

            /* Grab the most recent assignment row for this employee */
            ;WITH lastRow AS
            (
                SELECT TOP (1)
                       ea.Employee_ID, ea.Role_ID, ea.Location_ID,
                       ea.Compensation, ea.Active, ea.Reference_Date
                FROM dbo.Employee_Assignment ea
                WHERE ea.Employee_ID = @Employee_ID
                ORDER BY ea.Reference_Date DESC, ea.Employee_ID DESC
            )
            SELECT @OldComp    = Compensation,
                   @OldActive  = Active,
                   @RoleID     = Role_ID,
            ...
            IF @RoleID IS NULL
            BEGIN
                -- No assignment row found: nothing to update/audit
                RAISERROR('Employee %s has no assignment row.', 16, 1, @Employee_ID);
            END

            /* Update that most recent row */
            ;WITH lastRow AS
            (
                SELECT TOP (1) *
                FROM dbo.Employee_Assignment
                WHERE Employee_ID = @Employee_ID
                ORDER BY Reference_Date DESC, Employee_ID DESC
            )
            UPDATE lastRow
               SET Compensation  = @NewCompensation,
                   Active        = @NewActive,
                   Reference_Date= @refDate;

            /* Audit */
            INSERT dbo.Compensation_Audit
            (
                Employee_ID, Old_Compensation, New_Compensation,
                Old_Active, New_Active, Role_ID, Location_ID,
                Changed_By, Machine_Name, App_Name, Reason
            )
            VALUES
            (
                @Employee_ID, @OldComp, @NewCompensation,
                @OldActive, @NewActive, @RoleID, @LocationID,
                @ChangedBy, @MachineName, @AppName, @Reason
            );
...
SELECT
    CASE
        WHEN e.Years_of_Experience < 1 THEN '0 1 years'
        WHEN e.Years_of_Experience < 2 THEN '1 2 years'
        WHEN e.Years_of_Experience < 3 THEN '2 3 years'
        WHEN e.Years_of_Experience < 4 THEN '3 4 years'
        WHEN e.Years_of_Experience < 5 THEN '4 5 years'
        WHEN e.Years_of_Experience < 6 THEN '5 6 years'
        WHEN e.Years_of_Experience < 7 THEN '6 7 years'
        WHEN e.Years_of_Experience < 8 THEN '7 8 years'
        WHEN e.Years_of_Experience < 9 THEN '8 9 years'
        WHEN e.Years_of_Experience < 10 THEN '9 10 years'
        ELSE '10+ years'
...
SELECT 
    l.Location_Name, r.Role_Name,
    COUNT(CASE WHEN ea.Active = 'N' THEN 1 END) AS Inactive_Count,
    COUNT(*) AS Total_Employees,
    ROUND(100.0 * COUNT(CASE WHEN ea.Active = 'N' THEN 1 END) / COUNT(*), 2) AS Attrition_Rate
FROM Employee_Assignment ea
JOIN Role r ON ea.Role_ID = r.Role_ID
JOIN Location l ON ea.Location_ID = l.Location_ID
GROUP BY l.Location_Name, r.Role_Name
ORDER BY Attrition_Rate DESC;

SELECT 
    r.Role_Name,
    COUNT(*) AS Employee_Count,
    AVG(ea.Compensation) AS Avg_Salary,
    ROUND(AVG(ea.Compensation) * 0.10, 0) AS Bonus_Per_Employee,
    ROUND(SUM(ea.Compensation) * 0.10, 0) AS Total_Bonus_Cost
FROM 
    dbo.Employee ed
JOIN 
    dbo.Employee_Assignment ea ON ed.Employee_ID = ea.Employee_ID
JOIN 
    dbo.Role r ON ea.Role_ID = r.Role_ID
WHERE 
    r.Role_Name IN ('Senior Associate', 'Manager')
    AND ea.Active = 'Y'
GROUP BY 
    r.Role_Name;
